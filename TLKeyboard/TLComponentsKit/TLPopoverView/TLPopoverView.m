...
static CGFloat const kTLPopoverViewMargin = 8.f;       ///< 边距
static CGFloat const kTLPopoverViewCellHeight = 40.f;  ///< cell指定高度
static CGFloat const kTLPopoverViewArrowHeight = 13.f; ///< 箭头高度

static NSString *kPopoverCellReuseId = @"_PopoverCellReuseId";

float TLPopoverViewDegreesToRadians(float angle)
{
    return angle * M_PI / 180;
}


@interface TLPopoverView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<TLPopoverAction *> *actions;
...
- (void)layoutSubviews
{
    ...
    _tableView.frame = CGRectMake(0, _isUpward ? kTLPopoverViewArrowHeight : 0,
                                  CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kTLPopoverViewArrowHeight);
}
...
- (void)setStyle:(TLPopoverViewStyle)style
{
    _style = style;

    _tableView.separatorColor = [TLPopoverViewCell bottomLineColorForStyle:_style];

    if (_style == TLPopoverViewStyleDefault) {
...
- (void)initialize
{
    ...
    _actions = @[];
    _isUpward = YES;
    _style = TLPopoverViewStyleDefault;
    _arrowStyle = TLPopoverViewArrowStyleRound;
    ...
    [_tableView registerClass:[TLPopoverViewCell class] forCellReuseIdentifier:kPopoverCellReuseId];
...
- (void)showToPoint:(CGPoint)toPoint
{
    ...
    // 如果是菱角箭头的话, 箭头宽度需要小点.
    if (_arrowStyle == TLPopoverViewArrowStyleTriangle) {
        arrowWidth = 15.0;
    }

    // 如果箭头指向的点过于偏左或者过于偏右则需要重新调整箭头 x 轴的坐标
    CGFloat minHorizontalEdge = kTLPopoverViewMargin + cornerRadius + arrowWidth / 2 + 2;
    ...
    // 箭头高度
    currentH += kTLPopoverViewArrowHeight;

    // 限制最高高度, 免得选项太多时超出屏幕
    CGFloat maxHeight = _isUpward ? (_windowHeight - toPoint.y - kTLPopoverViewMargin) : (toPoint.y - CGRectGetHeight([UIApplication sharedApplication].statusBarFrame));
    ...
    // x: 窗口靠左
    if (toPoint.x <= currentW / 2 + kTLPopoverViewMargin) {
        currentX = kTLPopoverViewMargin;
    }
    // x: 窗口靠右
    if (_windowWidth - toPoint.x <= currentW / 2 + kTLPopoverViewMargin) {
        currentX = _windowWidth - kTLPopoverViewMargin - currentW;
    }
    // y: 箭头向下
    if (!_isUpward) {
    ...
    CGFloat maskTop = _isUpward ? kTLPopoverViewArrowHeight : 0;                                       // 顶部Y值
    CGFloat maskBottom = _isUpward ? currentH : currentH - kTLPopoverViewArrowHeight;                  // 底部Y值
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, cornerRadius + maskTop)
                        radius:cornerRadius
                    startAngle:TLPopoverViewDegreesToRadians(180)
                      endAngle:TLPopoverViewDegreesToRadians(270)
                     clockwise:YES];
    // 箭头向上时的箭头位置
    if (_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth / 2, kTLPopoverViewArrowHeight)];
        // 菱角箭头
        if (_arrowStyle == TLPopoverViewArrowStyleTriangle) {
            ...
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, arrowCornerRadius)
                             controlPoint:CGPointMake(arrowPoint.x - arrowWidth / 2 + arrowBottomCornerRadius, kTLPopoverViewArrowHeight)];
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, arrowCornerRadius)
                             controlPoint:arrowPoint];
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowWidth / 2, kTLPopoverViewArrowHeight)
                             controlPoint:CGPointMake(arrowPoint.x + arrowWidth / 2 - arrowBottomCornerRadius, kTLPopoverViewArrowHeight)];
        }
    }
    // 右上圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:TLPopoverViewDegreesToRadians(270)
                      endAngle:TLPopoverViewDegreesToRadians(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:TLPopoverViewDegreesToRadians(0)
                      endAngle:TLPopoverViewDegreesToRadians(90)
                     clockwise:YES];
    // 箭头向下时的箭头位置
    if (!_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x + arrowWidth / 2, currentH - kTLPopoverViewArrowHeight)];
        // 菱角箭头
        if (_arrowStyle == TLPopoverViewArrowStyleTriangle) {
            [maskPath addLineToPoint:arrowPoint];
            [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth / 2, currentH - kTLPopoverViewArrowHeight)];
            ...
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, currentH - arrowCornerRadius)
                             controlPoint:CGPointMake(arrowPoint.x + arrowWidth / 2 - arrowBottomCornerRadius, currentH - kTLPopoverViewArrowHeight)];
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, currentH - arrowCornerRadius)
                             controlPoint:arrowPoint];
            [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowWidth / 2, currentH - kTLPopoverViewArrowHeight)
                             controlPoint:CGPointMake(arrowPoint.x - arrowWidth / 2 + arrowBottomCornerRadius, currentH - kTLPopoverViewArrowHeight)];
        }
    }
    // 左下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:TLPopoverViewDegreesToRadians(90)
                      endAngle:TLPopoverViewDegreesToRadians(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
...
{
    CGFloat maxWidth = 0.f, titleLeftEdge = 0.f, imageWidth = 0.f, imageMaxHeight = kTLPopoverViewCellHeight - TLPopoverViewCellVerticalMargin * 2;
    CGSize imageSize = CGSizeZero;
    UIFont *titleFont = [TLPopoverViewCell titleFont];

    for (TLPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;

        if (action.image) { // 存在图片则根据图片size和图片最大高度来重新计算图片宽度

            titleLeftEdge = TLPopoverViewCellTitleLeftEdge; // 有图片时标题才有左边的边距
...
#pragma GCC diagnostic pop
        ...
        CGFloat contentWidth = TLPopoverViewCellHorizontalMargin * 2 + imageWidth + titleLeftEdge + titleWidth;
        if (contentWidth > maxWidth) {
            maxWidth = ceil(contentWidth); // 获取最大宽度时需使用进一取法, 否则Cell中没有图片时会可能导致标题显示不完整.
        }
    }

    // 如果最大宽度大于(窗口宽度 - kTLPopoverViewMargin*2)则限制最大宽度等于(窗口宽度 - kTLPopoverViewMargin*2)
    if (maxWidth > CGRectGetWidth(_keyWindow.bounds) - kTLPopoverViewMargin * 2) {
        maxWidth = CGRectGetWidth(_keyWindow.bounds) - kTLPopoverViewMargin * 2;
    }
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<TLPopoverAction *> *)actions
{
    ...
    // 弹窗在 pointView 底部
        ...
        toPoint.y = CGRectGetMaxY(pointViewRect) + 15;
    }
...
{
    return kTLPopoverViewCellHeight;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    TLPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellReuseId];
    cell.style = _style;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    [UIView animateWithDuration:0.25f animations:^{
        ...
        TLPopoverAction *action = self->_actions[indexPath.row];
