...
- (void)setShowModelBtn:(BOOL)showModelBtn
{
    _showModelBtn = showModelBtn;
    if (showModelBtn) {
        [self.modeButton mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)modeButtonDown
{
    ...
    if (self.status == TLChatBarStatusModelChange) {
        if (textRec.length > 0) {
        ...
        [self.modeButton setImage:kModeImage imageHL:kModeImageHL];
        self.status = TLChatBarStatusInit;
    } else {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:TLChatBarStatusModelChange];
        }
        if (self.status == TLChatBarStatusKeyboard) {
            [self.textView resignFirstResponder];
        ...
        [self.modeButton setImage:kModeImage imageHL:kModeImageHL];
        self.status = TLChatBarStatusModelChange;
    }
}
...
- (void)p_addMasonry
{
    ...
    [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(HEIGHT_CHATBAR_TEXTVIEW).priorityLow();
    }];
...
- (void)p_initImage
{
    ...
    kModeImage = [UIImage imageNamed:@"keyMenu"];
    kModeImageHL = [UIImage imageNamed:@"keyMenu"];
}
...
- (UIButton *)modeButton
{
    if (_modeButton == nil) {
        ...
        [_modeButton setImage:kModeImage imageHL:kModeImageHL];
